...
4        CONTINUE
      If (qc.eq.1) Write (91,*) 'Monotonic winds in RICH'
      If (NLVLS.lt.3 .or. NLVLS.gt.500) Then
          Write (91,*) NLVLS,' sounding levels in RICH.'
          qc=1
      End If
      If (NW.lt.3 .or. NW.gt.500) Then
          Write (91,*) NW,' wind levels in RICH.'
          qc=1
      End If
      DO 5 I=1,NW
      If (RHOW(I).le.0.0) Then
          Write (91,*) 'Bad value for RHOW ',I,RHOW(i)
          qc=1
          Goto 6
      End If
5     CONTINUE
6     CONTINUE
      ...
      If (HW(I)-HW(I-1).le.0.0) Then
          Write (91,*) 'Bad wind heights ',I,HW(I-1),HW(I)
          qc=1
          Goto 8
      End If
7     CONTINUE
8     CONTINUE
      ...
      Write (91,*) 'Bad value for RHO ',I,RHOW(i)
          qc=1
          Goto 16
      End If
15    CONTINUE
16    CONTINUE
      ...
      If (HT(I)-HT(I-1).le.0.0) Then
          Write (91,*) 'Bad sounding heights ',I,HT(I-1),HT(I)
          qc=1
          Goto 18
      End If
17    CONTINUE
...
101   SUMU=SUMU+RULAY*DZ
      ...
      ELSE
          ...
          VL=SUMVL/SUMRL
      END IF
c      Write (91,*) 'IN RICH BUOY,UL,VL,U6,V6'
c      Write (91,*) BUOY,UL,VL,U6,V6
C
