...
def test_ptozsa():
    ...
    test_out = np.array ([  1947.53320312,  11777.66894531,  20567.74609375,    110.40789795,
...
def test_ztopsa():
    zin = np.array ([  1947.53320312,  11777.66894531,  20567.74609375,    110.40789795,
         5571.04736328,  3010.15991211], dtype = 'float32')
    pout = np.zeros_like(zin)

    test_out = np.array ([  799.99993896,   200.00001526,    49.99999237,  1000.00024414,
...
def test_dzdlnp():
    ...
    test_out = np.array([  8868.51464844,   8838.07519531,   5824.51318359,   5853.78125,
...
def test_radnorm():
    ...
    test_out = np.array([ 1.01283681,  1.03501987,  1.03505003,  0.96664751,  1.00200331,
                          0.99825764], dtype='float32')
...
def test_soldec():
    ...
    test_out = np.array([-0.21854927, -0.40369907, -0.40244898,  0.40451825,  0.06728737,
                          -0.05679472], dtype='float32')
...
def test_timeq():
    ...
    test_out = np.array([ 0.07059176, -0.01070544, -0.012672  , -0.01510747, -0.0204956 ,
        0.04712578], dtype='float32')
...
def test_tdofesat():
    ...
    test_out = np.array([ 299.99993896,  299.00003052,  319.99996948,  230.00004578,
        274.        ,  275.        ], dtype='float32')
...
def test_pottemp():
    ...
    while not it.finished:
        ptw[it.index] = aoslib.pottemp(temp[it.index],dwpt[it.index],pres[it.index],1)
        pti[it.index] = aoslib.pottemp(temp[it.index],dwpt[it.index],pres[it.index],-1)
        it.iternext()
    if verbose:
        print("pottemp:")
        print(ptw)
...
def test_thetawa():
    ...
    ptw = np.zeros_like(temp)
    pti = np.zeros_like(temp)
    test_ptw = np.array([ 297.34692383,  304.38040161,  227.4776001 ,  201.44902039,
       -999.        ,   32.94946289], dtype='float32')
    test_pti = np.array([ 297.35095215,  304.37991333,  227.4776001 ,  201.44902039,
       -999.        ,   32.94946289], dtype='float32')

    it = np.nditer(temp, flags=['c_index'])
    while not it.finished:
        ptw[it.index] = aoslib.thetawa(temp[it.index],dwpt[it.index],pres[it.index],1)
        pti[it.index] = aoslib.thetawa(temp[it.index],dwpt[it.index],pres[it.index],-1)
        it.iternext()
    if verbose:
        print("thetawa:")
        print(ptw)
...
def test_cclpar():
    ...
    h = np.array([1542.80444336,3010.15991211,5571.04736328,7181.19189453,9158.66503906,
                  10357.0205078,11777.6689453,13601.7753906,16172.7070312,18434.2773438,
                  20567.7460938,23806.7363281,26377.6679688], dtype ='float32')
    t = np.array([ 296.6,  287.0,  266.9, 257.5,  242.1,  231.1,  220.7, 206.5,  204.5,    
                   212.3, 216.7, 224.5, 223.3 ],dtype ='float32')
