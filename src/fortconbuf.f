...
C Note: a patch to the Fortran Compiler performed during Apr 1995 requires
C this to be compiled with the +e option, or else Byte variable are treated
...
C                         contours and seed is minimum contour increment.
...
c -- unexpected behavior likely) which is likely for very large loops with
...
c
c     History
c     08-02-10 Qinglu Lin (Keane/SEC) DR20733 Change 2000 to 16000 
c                         for mp.
c
      Implicit None
c declare arguments
...
c
      ...
      Parameter (mwi=1500,mnc=200,mp=16000)
